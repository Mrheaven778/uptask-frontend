...
import { deleteTask, getTaksById } from "@/api/ProjectAPI";
...
function TaskCard({ task, resetdata }: TaskCardProps) {
  ...
  const fetchTask = async () => {
    const data = await getTaksById(task.id || "");
  };
  useEffect(() => {
    fetchTask();
  }, [task]);
    ...
    <>
      <li className="p-5 bg-gray-800 rounded border border-gray-700 flex justify-between gap-3">
        <div className="min-w-0 flex flex-col gap-y-4">
          <button
            type="button"
            className="text-xl font-bold text-slate-300 text-left"
          >
            {task.name}
          </button>
            ...
            >
              <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-gray-800 py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                <Menu.Item>
                  <button
                    type="button"
                    className="block px-3 py-1 text-sm leading-6 text-gray-100"
